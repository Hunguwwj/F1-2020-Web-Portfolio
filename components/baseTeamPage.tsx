...
========================================================= */
export default function BaseTeamPage({
  data,
  SceneComponent,
}: BaseTeamPageProps) {
  ...
  return (
    ...
    >
      <div className="team-car-scroll pointer-events-none fixed left-0 top-0 z-0 h-full w-screen will-change-transform">
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ backgroundColor: `${data.theme.accent}4D` }}
      />
  ...
  return (
    ...
    >
      <div className="relative flex h-full w-full overflow-hidden">
        <aside
          className="relative z-20 h-full w-[120px] shrink-0 overflow-hidden border-r ]"
          style={{
        ...
        >
          ...
          />
          <div
            className="absolute right-0 top-0 z-20 flex h-24 w-24 items-center justify-center"
...
function VerticalTeamTicker({ text, color }: { text: string; color: string }) {
  ...
  return (
    <div ref={containerRef} className="absolute inset-0 overflow-hidden">
      <div
        ref={trackRef}
        className="absolute left-1/2 top-0 flex flex-col will-change-transform -translate-x-1/2"
      >
      ...
      >
        ...
        <ul className="space-y-3 text-gray-800">
          {driver.stats.map(([label, value]) => {
            const isImportant =
              label === "Total Races" || label === "Total Points";
            return (
              <li
                key={label}
              ...
              >
                ...
                <span
                  ...
                  style={isImportant ? { color: data.theme.accent } : undefined}
                >
                  {value}
                </span>
              </li>
      ...
      >
        ...
        <ul className="space-y-2 text-base font-medium">
          {highlight.items.map(([label, value, note]) => (
            <li key={label}>
              {label}: <span className="text-xl font-bold">{value}</span> {note}
            </li>
          ))}
        </ul>
      </div>
  ...
  return (
    <Link href={href} target="_blank" className="group relative z-10 flex">
      <div
        ...
        onMouseLeave={(e) => {
          e.currentTarget.style.backgroundColor = isLarge
            ? "transparent"
            : "#111111";
      ...
      >
        <svg
          className={
            isLarge ? "ml-2 h-10 w-10 text-white" : "ml-2 h-8 w-8 text-white"
          }
          fill="currentColor"
          viewBox="0 0 24 24"
        >
...
function setupHorizontalGallery(pageElement: HTMLElement) {
  const track = pageElement.querySelector(
    ...
    { x: () => window.innerWidth * ANIMATION.gallery.startXRatio },
    {
      ...
      scrollTrigger: {
        ...
        start: "top top",
          ...
          `+=${track.scrollWidth * ANIMATION.gallery.scrollLengthMultiplier}`,
...
function setupCarDetailAnimations(pageElement: HTMLElement) {
  const root = pageElement.querySelector(
    SELECTORS.carDetailRoot,
  ) as HTMLElement | null;
  if (!root) return;
  const anims = [
    { selector: ".gsap-fade-up", vars: { y: ANIMATION.carDetails.fadeUpY } },
    { selector: ".gsap-slide-left", vars: { x: -ANIMATION.carDetails.slideX } },
    { selector: ".gsap-slide-right", vars: { x: ANIMATION.carDetails.slideX } },
    { selector: ".gsap-scale-up", vars: { y: 80 } },
  ];
